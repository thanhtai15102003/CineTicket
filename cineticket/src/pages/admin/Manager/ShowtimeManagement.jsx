...
import { Clock, Plus, Pencil, Trash2, Calendar, Ticket, Info } from 'lucide-react';
...
export default function ShowtimeManagement() {
    ...
    const calculateDuration = (startTime, endTime) => {
        ...
        if (endMins < startMins) endMins += 24 * 60; // Vắt qua rạng sáng
        return endMins - startMins;
    };

    /* ================= HÀM XỬ LÝ TRẠNG THÁI THỜI GIAN THỰC ================= */
    // 💡 Frontend tự tính toán xem phim đã chiếu xong chưa dựa trên giờ máy tính
    const getRealtimeStatus = (showDate, startTimeStr, endTimeStr, apiStatus) => {
        if (apiStatus === 'cancelled') return 'cancelled';

        const now = new Date();
        // Khởi tạo ngày giờ chiếu từ dữ liệu API
        const start = new Date(`${showDate}T${startTimeStr}`);
        const end = new Date(`${showDate}T${endTimeStr}`);

        // Nếu giờ kết thúc < giờ bắt đầu (VD: 23:00 - 01:30), cộng thêm 1 ngày cho giờ kết thúc
        if (end < start) {
            end.setDate(end.getDate() + 1);
        }

        // So sánh với giờ hiện tại
        if (now > end) return 'completed'; // Đã chiếu xong
        if (now >= start && now <= end) return 'on_showing'; // Đang chiếu

        // Nếu chưa chiếu, giữ nguyên trạng thái API (upcoming, active, inactive)
        return ['active', 'upcoming'].includes(apiStatus) ? 'upcoming' : apiStatus;
    };
    ...
    const fetchShowtimes = async () => {
        ...
        try {
            ...
            rawData.forEach((item) => {
                ...
                // Tính toán trạng thái thực tế ngay khi load
                const realStatus = getRealtimeStatus(
                    selectedDate,
                ...
                groupedMovies[movieIdStr].showtimes.push({
                    ...
                    status: realStatus // 🌟 Sử dụng trạng thái realtime đã tính
                });
            });
    ...
    /* ================= HELPER UI: BADGE TRẠNG THÁI ================= */
    const renderStatusBadge = (status) => {
        if (['upcoming', 'active'].includes(status)) {
            return (
                <span className="bg-blue-50 text-blue-600 border border-blue-200 px-2.5 py-1 rounded uppercase tracking-wide text-[10px] font-bold">
                    Sắp chiếu
                </span>
            );
        }
        if (['on_showing', 'now_showing'].includes(status)) {
            return (
                <span className="bg-orange-50 text-orange-600 border border-orange-200 px-2.5 py-1 rounded uppercase tracking-wide text-[10px] font-bold">
                    Đang chiếu
                </span>
            );
        }
        if (['completed', 'finished'].includes(status)) {
            return (
                <span className="bg-gray-100 text-gray-500 border border-gray-200 px-2.5 py-1 rounded uppercase tracking-wide text-[10px] font-bold">
                    Đã kết thúc
                </span>
            );
        }
        if (status === 'cancelled') {
            return (
                <span className="bg-red-50 text-red-600 border border-red-200 px-2.5 py-1 rounded uppercase tracking-wide text-[10px] font-bold">
                    Đã hủy
                </span>
            );
        }
        if (status === 'inactive') {
            return (
                <span className="bg-zinc-100 text-zinc-600 border border-zinc-200 px-2.5 py-1 rounded uppercase tracking-wide text-[10px] font-bold">
                    Tạm ngưng
                </span>
            );
        }
        return (
            <span className="bg-gray-100 text-gray-600 px-2.5 py-1 rounded uppercase text-[10px] font-bold">
                {status}
            </span>
        );
    };
    ...
    const toggleActive = async (movieId, showtimeId, ticketsSold, currentStatus) => {
        // CHẶN: Không cho phép thao tác nếu phim ĐÃ/ĐANG chiếu
        if (['completed', 'finished', 'on_showing', 'now_showing'].includes(currentStatus)) {
            showToast('Suất chiếu đã/đang diễn ra, không thể thay đổi!', 'warning');
            return;
        }

        const isCurrentlyActive = ['active', 'upcoming'].includes(currentStatus);

        // 💡 ĐÃ SỬA Ở ĐÂY: Gửi 'active' để mở lại suất chiếu thay vì 'upcoming'
        const newStatus = isCurrentlyActive ? 'cancelled' : 'active';

        if (isCurrentlyActive && ticketsSold > 0) {
            const confirmCancel = window.confirm(
                `Suất chiếu này ĐÃ BÁN ĐƯỢC ${ticketsSold} VÉ.\nViệc ĐÓNG suất chiếu đồng nghĩa với việc HỦY SUẤT.\nBạn có CHẮC CHẮN muốn thực hiện không?`
            );
            if (!confirmCancel) return;
        } else if (!isCurrentlyActive) {
            const confirmReopen = window.confirm('Bạn muốn khôi phục lại suất chiếu này?');
            ...
            prev.map((m) => {
                ...
                return {
                    ...
                    showtimes: m.showtimes.map((s) =>
                        s.id === showtimeId ? { ...s, status: newStatus } : s
                    )
                };
            })
        );
        ...
        try {
            // 💡 ĐÃ XÓA TRIỆT ĐỂ "/status" Ở CUỐI LINK
            const res = await fetch(
                `https://cinema-api-production-f2bc.up.railway.app/api/v1/manager/showtimes/${showtimeId}`,
                {
                    method: 'PATCH', // 💡 Lưu ý: Thường update 1 trường (status) BE sẽ dùng PATCH. Nếu test vẫn lỗi 405 Method Not Allowed thì bạn đổi chữ 'PATCH' thành 'PUT' nhé!
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${getToken()}`
                    },
                    body: JSON.stringify({ status: newStatus }) // Gửi 'active' hoặc 'cancelled'
                }
            );

            if (!res.ok) {
                const errJson = await res.json();
                throw new Error(errJson.message || 'Lỗi Server');
            }

            showToast(
                newStatus === 'cancelled'
                    ? 'Đã hủy suất chiếu thành công!'
                    : 'Đã mở lại suất chiếu!'
            );
        } catch (error) {
            // Khôi phục lại UI nếu Server lỗi
            setData((prev) =>
                prev.map((m) => {
                    if (m.id !== movieId) return m;
                    return {
                        ...m,
                        showtimes: m.showtimes.map((s) =>
                            s.id === showtimeId ? { ...s, status: currentStatus } : s
                        )
                    };
                })
            );
            showToast(`Lỗi Backend: ${error.message} ❌`, 'error');
        }
    };

    /* ================= EDIT ================= */
    const handleEdit = (movieId, showtimeId, ticketsSold, currentStatus) => {
        if (
            ['completed', 'finished', 'on_showing', 'now_showing', 'cancelled'].includes(
                currentStatus
            )
        ) {
            showToast('Không thể sửa suất chiếu đang chiếu, đã kết thúc hoặc bị hủy!', 'error');
            return;
        }
        if (ticketsSold > 0) {
    ...
    /* ================= DELETE (API) ================= */
    const handleDelete = async (movieId, showtimeId, ticketsSold, currentStatus) => {
        if (['completed', 'finished', 'on_showing', 'now_showing'].includes(currentStatus)) {
            showToast('Không thể xóa suất chiếu đã hoặc đang diễn ra!', 'error');
            return;
        }

        if (ticketsSold > 0) {
            showToast(
                'Không thể xóa suất chiếu đã có vé bán ra! Hãy dùng chức năng Hủy suất.',
                'error'
            );
            return;
        }
        if (!window.confirm('Xoá vĩnh viễn suất chiếu này? Hành động này không thể hoàn tác.'))
            return;
                            ...
                            {expanded[movie.id] && (
                                <div className="mt-5 border-t border-dashed border-gray-200 pt-4 space-y-3 animate-fade-in">
                                    ...
                                    {movie.showtimes.map((s) => {
                                        const isShowingOrDone = [
                                            'completed',
                                            'finished',
                                            'on_showing',
                                            'now_showing'
                                        ].includes(s.status);
                                        const isUpcoming = ['active', 'upcoming'].includes(
                                            s.status
                                        );
                                        ...
                                        return (
                                            <div
                                                key={s.id}
                                                className={`flex flex-col xl:flex-row xl:items-center justify-between p-4 rounded-xl gap-4 border transition-colors ${
                                                    isUpcoming
                                                        ? 'bg-gray-50/50 border-gray-100 hover:border-gray-300'
                                                        : 'bg-gray-100/70 border-gray-200 opacity-75'
                                                }`}
                                            >
                                                ...
                                                <div className="flex flex-wrap items-center gap-4 lg:gap-6">
                                                    ...
                                                    <div className="flex items-center gap-1 text-sm font-bold text-red-600 w-32">
                                                        <Ticket size={16} />
                                                ...
                                                <div className="flex items-center gap-5 border-t xl:border-t-0 pt-3 xl:pt-0">
                                                    <div className="text-xs font-medium text-gray-500 w-32 xl:text-right">
                                                        Đã bán:{' '}
                                                        <span
                                                    ...
                                                    <button
                                                        ...
                                                        title={
                                                            isShowingOrDone
                                                                ? 'Suất chiếu đã/đang diễn ra'
                                                                : isCancelled
                                                                  ? 'Suất chiếu đã bị hủy'
                                                                  : s.tickets_sold > 0
                                                                    ? 'Không thể sửa khi đã có vé bán'
                                                                    : 'Chỉnh sửa'
                                                        }
                                                    >
                                                        <Pencil size={18} />
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={
                                                            s.tickets_sold > 0 || isShowingOrDone
                                                        }
                                                        className={`p-1.5 rounded-lg transition-all ${
                                                            s.tickets_sold > 0 || isShowingOrDone
                                                                ? 'text-gray-300 cursor-not-allowed'
                                                        ...
                                                        title={
                                                            isShowingOrDone
                                                                ? 'Không thể xóa suất chiếu đã/đang diễn ra'
                                                                : s.tickets_sold > 0
                                                                  ? 'Hãy dùng nút gạt để Hủy suất thay vì Xóa'
                                                                  : 'Xóa vĩnh viễn'
                                                        }
                                                    >
                                                        <Trash2 size={18} />
